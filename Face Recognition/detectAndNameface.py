...
def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, text, clf):
    ...
    for (x, y, w, h) in face:
        ...
        if conf > 30:
            if id == 0:
                cv2.putText(img, "Mahmoud Mohy", (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255),1, cv2.LINE_AA)
            elif id == 1:
...
def recognize(img, clf, faceCascade):
    coords = draw_boundary(img, faceCascade, 1.1, 10, (0, 0, 0), "Face", clf)
    return img


faceCascade = cv2.CascadeClassifier("haarcascade_frontalface_default.xml")
clf = cv2.face.LBPHFaceRecognizer_create()
clf.read('runs/run_1/classifier.xml')
